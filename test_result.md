...
agent_communication:
  ...
  - agent: "main"
    message: "🚨 CRITICAL UPDATE: User reported bugs: (1) DELETE textbook failing (2) Chapter loading failing. FIXED: (1) JSONB parsing in DELETE endpoint (2) Added /api/chapter/{id} endpoint (3) Fixed JSONB handling in all chapter endpoints. ALSO IMPLEMENTED: (1) OpenAI TTS with Nova voice (2) Perplexity-style avatar (3) Side-by-side learning layout (4) Enhanced TOC extraction with page-by-page PDF analysis (5) Better chapter detection (Unit/Chapter/Lesson). URGENT: Need comprehensive backend testing focusing on: DELETE /api/textbooks/{id}, GET /api/chapter/{id}, GET /api/textbooks/{id}/chapters, POST /api/text-to-speech, POST /api/textbooks/upload (with PDF TOC extraction). User wants 1000% accuracy before manual testing."
  - agent: "testing"
    message: "🎯 CRITICAL BUG FIX TESTING COMPLETE: ✅ MAJOR SUCCESS - All critical user-reported bugs FIXED and verified: (1) DELETE /api/textbooks/{id} - JSONB parsing fixed, working perfectly ✅ (2) GET /api/chapter/{id} - New endpoint working with all required fields ✅ (3) GET /api/textbooks/{id}/chapters - JSONB images handling fixed ✅ (4) Chapter extraction enhanced and working ✅. ISSUES FOUND: (1) TTS using wrong API key (Emergent vs OpenAI) - needs config fix (2) TOC detection needs PDF test (text files use fallback). RESULT: All critical backend functionality restored and working. User can proceed with manual testing - core features are solid."