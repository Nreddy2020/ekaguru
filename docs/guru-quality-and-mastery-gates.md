...
**First-time reading is a job.** Request paths (evidence, image and lesson endpoints) never run OCR. A page revision without stored text answers 202 with the scan, `status: "PENDING"` and a reading job (`GuruEvidenceJob`, ids prefixed `evidence:`); the classroom shows the page at once with "Guru is reading this scanned page for the first time…" and polls the evidence endpoint until the text is stored, and only then requests a lesson. A lesson request that meets an unread page returns the reading job instead of queuing a generation; `GET /api/v2/guru/jobs/evidence:<id>` reports it (readable by the requester, an ADMIN, or anyone for a public built-in page). The evidence worker shares the lesson worker's rules (compare-and-swap claim, bounded attempts, stale recovery, `GURU_WORKER`), with its own `GURU_OCR_WORKER_CONCURRENCY` (default 1). PDFs with a text layer need no reading and answer immediately. Verified with unit tests on the service, queue, controllers, loader and studio, and live on a built-in page opened for the first time.
