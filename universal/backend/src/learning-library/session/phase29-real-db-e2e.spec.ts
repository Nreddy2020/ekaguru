...
describe('Phase 2.9 Real PostgreSQL DB E2E Runtime Journey Tests', () => {
  ...
  afterAll(async () => {
    ...
    // Clean up all seeded database records from the real PostgreSQL database to avoid pollution
    try {
  ...
  it('Verify Dynamic Session Lifecycle and Real Active Enrollment Calculations against Live PostgreSQL', async () => {
    ...
    const onboardRes = await request(app.getHttpServer())
      ...
      .send({ name: `RealLeo-${testId}`, learnerType: 'CHILD' })
      .expect(201);
    ...
    const getAssessmentRes = await request(app.getHttpServer())
      .get(`/api/v2/sessions/${sessionId}/assessments/${assessStep.assessmentInstances[0].id}`)
      .set('Authorization', `Bearer ${tokenUser}`)
      .expect(200);
    ...
    const respondRes = await request(app.getHttpServer())
      .post(`/api/v2/sessions/${sessionId}/assessments/${assessStep.assessmentInstances[0].id}/respond`)
      .set('Authorization', `Bearer ${tokenUser}`)
      .send({ response: '2' })
      .expect(200);
