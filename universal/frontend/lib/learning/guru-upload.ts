...
export async function connectLocalBook(bookId:string,learnerId:string,token:string,signal:AbortSignal,targetAudience?:BookAudience){
  ...
  const audience = targetAudience || book.targetAudience || 'child';
  ...
  form.append("targetAudience",audience);
  ...
  BookStorageService.updateBook({...book,targetAudience:audience,guruMaterial:{id:result.id,learnerId,sourceChecksum:checksum}});
