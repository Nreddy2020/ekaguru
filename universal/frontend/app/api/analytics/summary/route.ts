    ...
    try {
        ...
        if (action === 'final-push') {
            ...
            try {
                ...
                try {
                    execSync('git commit -m "fix(m2): resolve pdf-parse class interop and wire M2 engine to BookService"', { cwd: 'E:/Ekaguru' });
                } catch(e) {}
                out = execSync('git push origin feat/m2-document-intelligence-engine', { cwd: 'E:/Ekaguru' }).toString();
            } catch(err: any) {
                out = err.message + ' ' + (err.stderr ? err.stderr.toString() : '');
            }
            return NextResponse.json({ pushStatus: out }, { status: 200 });
        }
