...
function MathScratchpad({ onClear }: MathScratchpadProps) {
  ...
  return (
    <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 space-y-3">
      <div className="flex justify-between items-center text-xs text-slate-400 font-bold uppercase tracking-wider">
        <span>✏️ Mathematical Working Area (Scratchpad)</span>
        <button
          onClick={clearCanvas}
          className="px-2.5 py-1 bg-slate-900 border border-slate-800 hover:border-slate-700 text-[10px] text-slate-300 hover:text-white rounded"
        >
          Clear Board
        </button>
      </div>
      <canvas
        ...
        width={500}
        height={150}
        className="w-full h-[150px] bg-slate-950 border border-slate-900 rounded-lg cursor-crosshair touch-none"
        ...
        {currentStep === 5 && (
          <div className="space-y-8 w-full max-w-2xl mx-auto">
            ...
            <div className={`border-2 rounded-2xl bg-slate-900 p-8 shadow-2xl transition-all duration-300 ${
              misconceptionAlert ? "border-amber-500 bg-slate-900/90 shadow-amber-500/5" : "border-slate-800"
            }`}>
              
              {/* Reasoning-gap (misconception) warning header */}
              {misconceptionAlert && (
                <div className="mb-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl flex items-start gap-3 text-amber-400 text-sm animate-pulse">
                  <ShieldAlert className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" />
                  <div>
                    <div className="font-bold text-white mb-0.5">Let's investigate your reasoning</div>
              ...
              <div className="space-y-6">
                ...
                {/* Mathematical Scratchpad Canvas */}
                {showScratchpad && (
                ...
                <div className="flex flex-wrap justify-between items-center pt-6 border-t border-slate-800">
                  <div className="flex gap-2">
                    ...
                    >
                      ✏️ {showScratchpad ? "Hide Scratchpad" : "Show Scratchpad"}
                    </button>
