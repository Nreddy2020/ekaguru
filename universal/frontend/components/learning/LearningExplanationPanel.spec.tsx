...
it('retains extracted page content for notes',async()=>{render(<LearningExplanationPanel bookId="my-book" sourceAnchor={{pdfPage:6}}/>);await waitFor(()=>expect(screen.getAllByText('The heart pumps blood to the whole body.').length).toBeGreaterThan(0));expect(screen.queryByText(/BKT Confirmed/)).not.toBeInTheDocument();});
