...
export const VitalisSidebar: React.FC<VitalisSidebarProps> = ({
  ...
  return (
    <aside className="w-80 bg-[#09111f] border-r border-white/[0.1] p-5 flex flex-col justify-between h-[calc(100vh-4.75rem)] overflow-y-auto shrink-0 select-none space-y-6">
      <div className="space-y-6">
        {/* Upload Action */}
        <button
          onClick={onOpenUpload}
          className="w-full py-4 px-5 rounded-2xl bg-teal-500/20 border border-teal-500/40 text-teal-300 font-bold text-sm sm:text-base flex items-center justify-center gap-3 hover:bg-teal-500/30 transition-all shadow-md"
        >
        ...
        <div className="space-y-6">
          {navGroups.map((group) => (
            <div key={group.groupName} className="space-y-2">
              <div className="text-xs font-black text-slate-400 uppercase tracking-widest px-3">
                {group.groupName}
              </div>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl text-sm sm:text-base font-semibold transition-all text-left ${
                        isActive
                    ...
                    >
                      <div className="flex items-center gap-3.5">
                        <span className={`text-lg ${isActive ? 'text-teal-400 font-bold' : 'text-slate-400'}`}>
                          {item.icon}
                        </span>
                        <span className="truncate">{item.label}</span>
                      </div>
                      {item.badge !== undefined && item.badge > 0 && (
                        <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-rose-500/30 text-rose-300 border border-rose-500/50 font-mono">
                          {item.badge}
                        </span>
                      )}
                    </button>
      ...
      <div className="pt-6 pb-2 border-t border-white/[0.1] text-xs sm:text-sm text-slate-400 space-y-1.5 px-2">
        <div className="text-teal-400 font-bold flex items-center gap-2 text-sm sm:text-base">
          <span>🔒</span>
          <span>Safe Observation</span>
        </div>
        <p className="text-xs sm:text-sm text-slate-300 leading-snug">PII automatically masked before canonical indexing.</p>
      </div>
    </aside>
  );
};
